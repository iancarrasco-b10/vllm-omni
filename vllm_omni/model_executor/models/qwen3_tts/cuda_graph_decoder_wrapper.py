...
class CUDAGraphDecoderWrapper:
    ...
    Captures the decoder forward pass for fixed input sizes and replays them
    during inference to reduce kernel launch overhead.
    ...
    def warmup(self, device: torch.device, dtype: torch.dtype = torch.long):
        ...
        for size in self.capture_sizes:
            try:
                ...
                logger.warning(
                    "  Failed to capture CUDA Graph for size=%d",
                    size,
                    exc_info=True,
                )
