...
class CUDAGraphDecoderWrapper:
    ...
    def chunked_decode_with_cudagraph(
        ...
        while start_index < total_len:
            ...
            # Take expected sample count from the end to avoid over-trimming when
            # decoder returns fewer samples (pops/sample loss). See QwenLM/Qwen3-TTS#234
            sample_count = min(
                (end_index - start_index) * total_upsample,
                wav_chunk.shape[-1],
            )
