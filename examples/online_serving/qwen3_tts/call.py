...
def main():
    ...
    # Resolve reference audio: URLs pass through directly; local files
    # become file:// URIs (avoids base64 bloat that exceeds WS frame limits).
    ref_audio_uri = None
    if args.ref_audio:
        if args.ref_audio.startswith(("http://", "https://", "data:")):
            ...
            if not os.path.isfile(args.ref_audio):
                ...
                raise SystemExit(1)
            abs_path = os.path.abspath(args.ref_audio)
            ref_audio_uri = f"file://{abs_path}"
            size_kb = os.path.getsize(args.ref_audio) / 1024
            print(f"Reference audio: {abs_path} ({size_kb:.1f} KB)")
    ...
    if ref_audio_uri is not None:
        config["ref_audio"] = ref_audio_uri
